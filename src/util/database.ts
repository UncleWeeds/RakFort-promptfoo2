...
export async function deleteAllEvals(): Promise<void> {
  const db = getDb();
  db.transaction((tx) => {
    tx.delete(evalResultsTable).run();
    tx.delete(evalsToPromptsTable).run();
    tx.delete(evalsToDatasetsTable).run();
    tx.delete(evalsToTagsTable).run();
    tx.delete(evalsTable).run();
  });
}
