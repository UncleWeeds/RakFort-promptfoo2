...
evalRouter.get('/summaries', async (req: Request, res: Response): Promise<void> => {
  try {
    const summaries = await getEvalSummaries();
    res.json(summaries);
  } catch (error) {
    logger.error(`Failed to fetch eval summaries: ${error}`);
    res.status(500).json({ error: 'Failed to fetch eval summaries' });
  }
});
